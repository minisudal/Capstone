...
public class CategoryViewModel extends ViewModel {
    ...
    private final Map<Integer, String> checkboxTexts = new HashMap<>();
    private final Set<String> addedTexts = new LinkedHashSet<>();
    private final MutableLiveData<Set<String>> liveAddedTexts = new MutableLiveData<>(addedTexts);
    private final MutableLiveData<Boolean> itemSelected = new MutableLiveData<>(false);
    private int checkedItemCount = 0;

    public LiveData<Set<String>> getLiveAddedTexts() {
        return liveAddedTexts;
    }

    public LiveData<Boolean> isItemSelected() {
        return itemSelected;
    }
    ...
    public void setChecked(int checkboxId, boolean checked, String text) {
        ...
        checkboxStates.put(checkboxId, checked);
        if (checked) {
            if (!wasChecked) checkedItemCount++;
            addText(text);
        } else {
            if (wasChecked) checkedItemCount--;
            removeText(text);
        ...
        if (sharedViewModel != null) {
            if (checkedItemCount > 0) {
                itemSelected.setValue(true);
            } else {
                itemSelected.setValue(false);
            }
        } else {
            Log.e("CategoryViewModel", "sharedViewModel is null");
        }
    }

    private void removeText(String text) {
        addedTexts.remove(text);
        liveAddedTexts.setValue(new LinkedHashSet<>(addedTexts));
    }
